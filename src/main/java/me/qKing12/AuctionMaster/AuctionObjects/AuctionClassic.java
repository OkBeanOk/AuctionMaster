...
public class AuctionClassic implements Auction{
	...
	public boolean placeBid(Player player, double amount, int cacheBids){
		...
		// Fire PlaceBidEvent so other plugins can cancel / modify behavior
		PlaceBidEvent placeBidEvent = new PlaceBidEvent(player, this, amount);
		...
		// Determine previous bid from this player (if any)
		Bids.Bid lastBid = getLastBid(uuid);
		double previousAmount = lastBid == null ? 0D : lastBid.getCoins();

		// Extra coins that actually need to be reserved for this new bid
		double delta = amount - previousAmount;

		// If they are lowering their own bid (should normally not happen),
		// refund the difference immediately
		if (delta < 0D) {
			AuctionMaster.economy.addMoney(player, -delta);
		} else if (delta > 0D) {
			// Need more coins reserved for this new bid
			// Make sure the player has enough and then remove it
			if (!AuctionMaster.economy.hasMoney(player, delta)) {
				player.sendMessage(Utils.chat(AuctionMaster.bidsRelatedCfg.getString("not-enough-money")));
				return false;
			}

			if (!AuctionMaster.economy.removeMoney(player, delta)) {
				// If your Currency impl returns boolean, respect it;
				// otherwise you can drop this check.
				player.sendMessage(Utils.chat(AuctionMaster.bidsRelatedCfg.getString("not-enough-money")));
				return false;
			}
		}
		...
		if(AuctionMaster.auctionsHandler.bidAuctions.containsKey(uuid)){
			if(getLastBid(uuid)==null) {
				AuctionMaster.auctionsHandler.bidAuctions.get(uuid).add(this);
				AuctionMaster.auctionsDatabase.addToOwnBids(uuid, id);
			}
		}
		else{
			ArrayList<Auction> auctions = new ArrayList<>();
			auctions.add(this);
			AuctionMaster.auctionsHandler.bidAuctions.put(uuid, auctions);
			AuctionMaster.auctionsDatabase.addToOwnBids(uuid, id);
		}
